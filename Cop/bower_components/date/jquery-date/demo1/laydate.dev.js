...
//获取组件存放路径
    Dates.getPath = (function () {
        ...
        for (var i in jsPath) {
            if (js[i].src.indexOf('bower_components') != -1) {
                var a = js[i].src.indexOf('bower_components');
                jsPath = js[i].src.substring(0, a) + 'bower_components/' + 'date/jquery-date/demo1/';
                break;
            }
        }
...
//方位辨别
    Dates.orien = function (obj, pos) {
        ...
        var right = ($(window).width() - rect.left - obj.clientWidth);
        if (right <= 0) {
            obj.style.left = rect.left + (pos ? 0 : Dates.scroll(1)) + right + 'px';
        } else {
