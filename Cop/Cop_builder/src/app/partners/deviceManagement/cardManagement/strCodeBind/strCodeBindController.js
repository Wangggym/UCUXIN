...
app.controller('strCodeBindController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'applicationServiceSet', 'toastr', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, applicationServiceSet, toastr) {
    ...
    var controller = {
      ...
      getSchoolList: function () {
        ...
        //根据gid获取学生绑卡列表
        GetStuCardList: function (gid, name,ClassID) {
            applicationServiceSet.attendanceService.basicDataControlService.GetStuCardList.send([gid,ClassID, $scope.model.name, $scope.model.checked]).then(function (data) {
                if (data.Ret == 0) {
        ...
        basic: function () {
            ...
            $scope.search = function () {
                if ($scope.model.school) {
                    controller.GetStuCardList($scope.model.school, $scope.schoolName,$scope.model.class.ClassID);
                }
            };
            /*学校模糊查询*/
            ...
            $scope.goToBindCard = function (umid) {
                $state.go("access.app.partner.cardManagement.stuBindCard", {ID: umid})
            }
        }
    };
