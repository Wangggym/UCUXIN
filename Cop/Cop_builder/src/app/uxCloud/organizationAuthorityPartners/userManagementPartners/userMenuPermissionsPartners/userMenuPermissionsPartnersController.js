        ...
        serviceApi: (function () {
            return {
                ...
                getUserMenuPage: function () {
                    applicationServiceSet.internalServiceApi.userManagement.GetMenusByOrgUIDNew.send([APPMODEL.Storage.getItem('copPage_token'), APPMODEL.Storage.getItem("orgid"), $stateParams.UID,2]).then(function (data) {
                        if (data.Ret == 0) {
                ...
                addUserMenu:function (item) {
                    applicationServiceSet.internalServiceApi.userManagement.AddUserMenu.send([$stateParams.UID, APPMODEL.Storage.getItem('orgid'), item.MenuID,2]).then(function (data) {
                        if (data.Ret == 0) {
                            toastr.success('添加成功！');
                ...
                deleteUserMenu: function (item) {
                    applicationServiceSet.internalServiceApi.userManagement.DeleteUserMenuNew.send([item.MenuID,APPMODEL.Storage.getItem('orgid'),$stateParams.UID,2]).then(function (data) {
                        if (data.Ret == 0) {
                            toastr.success('删除成功');
                            userMenuPermissions.serviceApi.getUserMenuPage();
                        }
