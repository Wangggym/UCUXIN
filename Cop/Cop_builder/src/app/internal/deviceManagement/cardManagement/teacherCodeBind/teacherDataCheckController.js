 ...
 * teacherDataCheckContorller
 * 老师异常数据检查
 */
app.controller('teacherDataCheckContorller', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', 'applicationServiceSet', 'toastr', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, applicationServiceSet, toastr) {
    /**
     * 异常数据检查入口函数声明
     */
        ...
        var errorStuCardInfo = {
            ...
            errorInfo: function () {
                if (APPMODEL.Storage.getItem("searchObj")) {
                    ...
                    applicationServiceSet.internalServiceApi.cardManagement.getErrorStuCardInfo.send([APPMODEL.Storage.getItem("copPage_token"), $scope.searchObj.selectedGid]).then(function (data) {
                        if (data.Ret == 0) {
            ...
            bindData: function (data) {
                ...
                $scope.deleteCardInfo = function (card) {
                    var id = card.CardNoList[0].ID;
                    $scope.teacherList.splice($scope.teacherList.indexOf(card), 1);
