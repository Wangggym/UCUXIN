 ...
 */
app.controller('LiveRecordListController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    ...
    var  liveRecordList={
        ...
        init:function () {
            ...
            setTimeout(function () {
                ...
                liveRecordList.serviceApi.GetAnchorLiveNav();
            },500);
        },
        //变量
        variable:function () {
        ...
        serviceApi: (function () {
            return {
                //获取直播员列表
                GetAnchorLiveNav:function () {
                    applicationServiceSet.liveService.officialBackground.GetAnchorLiveNav.send([APPMODEL.Storage.getItem('copPage_token'),
                        $scope.model.uid, $scope.model.gid,$scope.model.gTypeID,  $scope.model.s_state,$scope.model.keyword,$scope.model.pubStartDate,$scope.model.pubEndDate,
                        $scope.model.pIndex,
                        $scope.model.pSize,$scope.model.sortFieldList,$scope.model.isMoke,$scope.model.OrgLevel,$scope.model.OrgID])
                        .then(function (data) {
                            if(data.Ret==0){
                                liveRecordList.setting.DataChange(data.Data)
                            }
                        });
                },
        ...
        operation:function () {
            ...
            $scope.Forbid = function (isForbid,item) {
                 ...
                 liveRecordList.serviceApi.Forbid(isForbid,[item.LiveID],function () {
                        if(isForbid){
                            item.Status=-1;
                            item.StatusName="禁播";
                        }
            ...
            $scope.ForbidAll = function (isForbid) {
                ...
                liveRecordList.serviceApi.Forbid(isForbid,ids,function () {
                    liveRecordList.serviceApi.GetAnchorLiveNav();
                    $scope.checkedAll=false;
                });
            }
            ...
            $scope.btnReturn = function () {
                history.back();
                // $location.url('access/app/internal/UXLive/UserList?activeType='+ 1);
            }
        },
        /**
         * 设置
