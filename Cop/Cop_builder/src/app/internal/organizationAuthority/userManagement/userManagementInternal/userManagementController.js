 ...
 * user management
 */
app.controller('userManagementController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    /**
     * 用户管理
    ...
    var userManagement = {
        ...
        serviceApi: (function () {
            return {
                 ...
                 */
                getOrgList: function () {
                    applicationServiceSet.internalServiceApi.userManagementInstitution.GetOrgList.send([APPMODEL.Storage.getItem('copPage_token'), 0]).then(function (data) {
                        if (data.Ret == 0) {
                            $scope.model.organizationList = data.Data;
                        }
                    });
                },
                /**
                ...
                getSimpleOrgUsers: function () {
                    console.log(applicationServiceSet)
                    applicationServiceSet.internalServiceApi.userManagement.GetOrgAdmin.send([APPMODEL.Storage.getItem('copPage_token'), APPMODEL.Storage.getItem('orgid'),1]).then(function (data) {
                        if (data.Ret == 0) {
                ...
                addOrgUser: function (item, func) {
                    ...
                    applicationServiceSet.internalServiceApi.userManagementInstitution.AddOrgUserNew.send([0, item.UID,1], [APPMODEL.Storage.getItem('copPage_token')]).then(function (data) {
                        if (data.Ret == 0) {
//                             var userItem = {
//                                 "UID": item.UID,
//                                 "UName": item.UName
                ...
                pageIndex: function () {
                    ...
                    $scope.pageIndex = {
                        ...
                        fliPage: function (page) {
                            applicationServiceSet.internalServiceApi.userManagementInstitution.GetOrgUserPage.send([APPMODEL.Storage.getItem('copPage_token'), $scope.model.organization, $scope.model.userId, $scope.model.pSize, page.pIndex]).then(function (data) {
                                if (data.Ret == 0) {
                                    userManagement.setting.dataChange(data.Data);//数据类型转换
                                }
                            });
                        ...
                        nextPage: function (pageNext) {
                            applicationServiceSet.internalServiceApi.userManagementInstitution.GetOrgUserPage.send([APPMODEL.Storage.getItem('copPage_token'), $scope.model.organization, $scope.model.userId, $scope.model.pSize, pageNext]).then(function (data) {
                                if (data.Ret == 0) {
                                    userManagement.setting.dataChange(data.Data);//数据类型转换
                                }
                            });
 ...
 */
app.controller('userManagementModalContentCtrl', ['$scope', '$modalInstance', 'items', 'service', 'enumOrgType', 'setting', 'toastr', function ($scope, $modalInstance, items, service, enumOrgType, setting, toastr) {
    /**
     * 模态框添加用户
