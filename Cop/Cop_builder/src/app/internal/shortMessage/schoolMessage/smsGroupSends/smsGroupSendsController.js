...
app.controller('SmsGroupSendsController', ['$scope','$filter','$location', '$modal', 'toastr', 'toastrConfig', 'applicationServiceSet', function ($scope,$filter, $location, $modal, toastr, toastrConfig, applicationServiceSet) {
  ...
  $scope.pagination = {
    ...
    $scope.export = function () {
      ...
      window.open(urlConfig+'public/v3/SmsWeb/ExportSmsSendList?token='+APPMODEL.Storage.copPage_token+'&bID=0'+'&rvName='+keywords+'&sDate='+sDate+'&eDate='+eDate);
    };
  var smsGroupSends = (function () {
    ...
    var getService = function (method, arr, list, fn) {
      ...
      applicationServiceSet.internalServiceApi.message[method].send(arr, list).then(fn);
    }
    ...
    var getSmsList = function () {
      ...
      getService('getSmsMassSendList', [APPMODEL.Storage.copPage_token, 0, keywords, $scope.pagination.currentPage, $scope.pagination.itemsPerPage,sDate,eDate], function (data) {
        if (data.Ret === 0) {
          if (data.Data) {
...
app.controller('AddSmsGroupSendCtrl', ['$scope', '$modalInstance', 'items', '$modal', 'applicationServiceSet', 'toastr', 'toastrConfig', function ($scope, $modalInstance, items, $modal, applicationServiceSet, toastr, toastrConfig) {
  ...
  $scope.refreshSchool = function (keyword) {
    if (!keyword) return;
    smsGroupSends.getSchool(keyword);
  };
  ...
  $scope.comfirm = function () {
    ...
    if ($scope.submitModelData.school) {
      $scope.pushData.rvItems.push({
        AttrType: 5,
        AttrValue: $scope.submitModelData.school.GID,
