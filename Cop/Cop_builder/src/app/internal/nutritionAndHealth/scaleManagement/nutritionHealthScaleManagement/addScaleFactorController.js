...
app.controller('addScaleFactorController', ['$scope', 'toastr', '$modal', 'applicationServiceSet', '$window','$location', function ($scope, toastr, $modal, applicationServiceSet, $window, $location) {
    ...
    function factorFunction() {
        ...
        this.onEvent = function () {
            ...
            $scope.next = function () {
                ...
                $location.path('access/app/internal/nutritionScalMannage/factorExplain');
            };
            // 跳转到题库
            $scope.prev = function () {
    ...
    factorFunction.prototype.upDateFactor = function (item) {
        applicationServiceSet.internalServiceApi.nutritionHealth.updateFactors.send([item.ID,item.ScaID,item.LstSqeNo,item.Name,item.Rule,item.Remark,item.StandardScore,item.Type,item.MinFirst,item.MaxFirst,item.MinSecond,item.MaxSecond,item.MinThird,item.MaxThird,item.Range]).then(function (data) {
            if(data.Ret == '0'){
...
app.controller('AddFactorDefinitionCtrl', ['$scope', '$modalInstance', 'items', 'toastr', 'toastrConfig', function ($scope, $modalInstance, items, toastr, toastrConfig) {
    ...
    $.each($scope.factorList,function (e, item) {
        item.MinRange = item.Range.split('~')[0];
        item.MaxRange = item.Range.split('~')[1];
    });
    // 配置提示消息框
    ...
    $scope.confirm = function () {
         ...
         * 因子类型是否为基本类型
         */
        if($scope.factor.Type == 0){
            $scope.factor.Range = '-100~-100';
            $.each($scope.factor,function (e,item) {
                if(e == 'MinFirst' || e == 'MaxFirst' || e == 'MinSecond' || e == 'MaxSecond' || e == 'MinThird' || e == 'MaxThird'){
                    if(!item){
                        item = -100;
                    }
                }
            ...
            if(!$scope.factor.MinRange || $scope.factor.MaxRange == ''|| !$scope.factor.MaxRange || $scope.factor.MaxRange == ''){
                toastr.error('总分必填并且必须为数字！');
                return;
            }
            $scope.factor.Range = $scope.factor.MinRange+'~'+$scope.factor.MaxRange;
        }
        $.each($scope.factor,function (e,item) {
            if(e == 'MinFirst' || e == 'MaxFirst' || e == 'MinSecond' || e == 'MaxSecond' || e == 'MinThird' || e == 'MaxThird'){
                if(!item){
        ...
        if(parseFloat($scope.factor.MinFirst)>parseFloat($scope.factor.MaxFirst) || parseFloat($scope.factor.MinSecond)>parseFloat($scope.factor.MaxSecond) || parseFloat($scope.factor.MinThird)>parseFloat($scope.factor.MaxThird) || parseFloat($scope.factor.MinRange)>parseFloat($scope.factor.MaxRange)){
            toastr.error('最低分不能高于最高分！');
            return ;
        }
