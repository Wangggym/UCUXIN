 ...
 */
app.controller('paymentAnnClasSstatisticsController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    /**
    ...
    var paymentBySchoolStatistics = {
        ...
        serviceApi: (function () {
            return {
                 ...
                 */
                GetChargeStuGroupByClassList: function () {
                    if(!$scope.queryFields.selectedGid) return  toastr.error('请输入学校')
                    applicationServiceSet.chargeServiceApi.chargeService.GetChargeStuGroupByClassList.send([
                        APPMODEL.Storage.getItem('copPage_token'),
                        $scope.queryFields.selectedGid,
                        $scope.model.selectProductPackageID,
                        $scope.model.openProductId,
                        paymentBySchoolStatistics.operation.dateChange($scope.model.dateStart),
                        paymentBySchoolStatistics.operation.dateChange($scope.model.dateOver)
                    ]).then(function (data) {
                        if (data.Ret == 0) {
        ...
        operation: (function () {
            ...
            $scope.refreshAddresses = function (selectedGid) {
                if (selectedGid) {
                    paymentBySchoolStatistics.serviceApi.getOrgSchoolPage(selectedGid);//get school org pages list
                }
            };
            /**
             * choose school
            ...
            return {
                ...
                basic: function () {
                    ...
                    $scope.search = function () {
                        paymentBySchoolStatistics.serviceApi.GetChargeStuGroupByClassList();//get school pay information statistic
                    };
                    /**
                     * 分页配置
                     */
                    ...
                    $scope.export = function () {
                        $window.location.href = urlConfig + 'Charge/v3/ChargeWeb/ExportChargeStuGroupByClassList' + '?token=' + APPMODEL.Storage.getItem("copPage_token") + '&orgid=' + $scope.model.partners + '&gid=' + $scope.queryFields.selectedGid + '&ProvinceId=' + $scope.model.province + '&CityId=' + $scope.model.city + '&CountyId=' + $scope.model.county + '&productId=' + $scope.model.selectProductPackageID + '&chargeId=' + $scope.model.openProductId + '&sDate=' + paymentBySchoolStatistics.operation.dateChange($scope.model.dateStart) + '&eDate=' + paymentBySchoolStatistics.operation.dateChange($scope.model.dateOver);
