 ...
 */
app.controller('schoolApplicationOpeningController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    /**
     * 学校应用开通
        ...
        serviceApi: (function () {
            return {
                ...
                getGroupSetList: function () {
                    applicationServiceSet.internalServiceApi.schoolApplicationManagement.GetGroupSetList.send([APPMODEL.Storage.getItem('copPage_token'), $scope.model.selectedGid, $scope.model.pSize, $scope.model.pIndex]).then(function (data) {
                        if (data.Ret == 0) {
                ...
                pageIndex: function () {
                    ...
                    $scope.pageIndex = {
                        ...
                        fliPage: function (page) {
                            applicationServiceSet.internalServiceApi.schoolApplicationManagement.GetGroupSetList.send([APPMODEL.Storage.getItem('copPage_token'), $scope.model.selectedGid, $scope.model.pSize, page.pIndex]).then(function (data) {
                                if (data.Ret == 0) {
                                    schoolApplicationOpening.setting.dataHandle(data.Data);//数据处理
                                }
                            });
                        ...
                        nextPage: function (pageNext) {
                            applicationServiceSet.internalServiceApi.schoolApplicationManagement.GetGroupSetList.send([APPMODEL.Storage.getItem('copPage_token'), $scope.model.selectedGid, $scope.model.pSize, pageNext]).then(function (data) {
                                if (data.Ret == 0) {
                                    schoolApplicationOpening.setting.dataHandle(data.Data);//数据处理
                                }
                            });
