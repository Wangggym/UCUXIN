...
app.controller('AddSmsPushController', ['$scope', '$location', 'applicationServiceSet', '$modal', 'toastr', 'toastrConfig', function ($scope, $location, applicationServiceSet, $modal, toastr, toastrConfig) {
    ...
    var addSmsPush = (function () {
        ...
        var getSchool = function (keyword) {
            applicationServiceSet.internalServiceApi.paymentMessage.getFuzzySchoolList.send([applicationToken, keyword]).then(function (data) {
                if (data.Ret == 0) {
    ...
    $scope.refreshSchool = function (keyword) {
        if (!keyword) return;
        addSmsPush.getSchoolList(keyword);
    };
