        ...
        serviceApi: (function () {
            return {
                ...
                fileUpload: function (files) {
                    applicationServiceSet.chargeServiceApi.chargeService.importMoney.fileUpload(files).then(function (data) {
                        if(data.Ret==0){
                            toastr.success("导入成功！")
                        }
                    });
                ...
                getPayStatisticByClass: function () {
                    applicationServiceSet.chargeServiceApi.chargeService.GetPayStatistic.send([APPMODEL.Storage.getItem('copPage_token'), $scope.pagination.itemsPerPage, $scope.pagination.currentPage,$scope.model.partners,$scope.model.gname]).then(function (data) {
                        if (data.Ret == 0) {
            ...
            return {
                ...
                basic: function () {
                    ...
                    $scope.export = function () {
                        $window.location.href = urlConfig + 'Charge/v3/Web/ExportOffLineDetail' + '?token=' + APPMODEL.Storage.getItem("copPage_token") + '&partnerName='+$scope.model.partners+'&partnerName='+$scope.model.gname;

                    };
                    //下载导入
                    $scope.exportDownload = function () {
                        $window.location.href = urlConfig + 'Charge/v3/Web/DownLoadOffLineDetailExcel' + '?token=' + APPMODEL.Storage.getItem("copPage_token") + '&partnerName='+$scope.model.partners+'&partnerName='+$scope.model.gname;
                    ...
                    $scope.fileUpload = function (files) {
                        console.log(files);
                        if(files){
                            paymentBySchoolStatistics.serviceApi.fileUpload(files);
                        }
                    };
