...
app.controller('FixedEntryPushController', ['$scope','$modal', '$location', 'applicationServiceSet', function ($scope,$modal, $location, applicationServiceSet) {
    ...
    var fixedEntryPush = (function () {
        ...
        var getPushRecord = function () {
            ...
            getService('getPushRecordPage', [token, $scope.pagination.currentPage, $scope.pagination.itemsPerPage, 3, 0, schoolId], function (data) {
  ...
  $scope.refreshSchool = function (keyword) {
    if (!keyword) return;
    fixedEntryPush.getSchoolList(keyword);
  };
    ...
    $scope.addPush = function () {
        $location.path('/access/app/internal/paymentMessage/addFixedEntryPush');
    };
