 ...
 */
app.controller('listOfCloudRegionMenu', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    /**
     * user menu permissions
        ...
        serviceApi: (function () {
            return {
                ...
                getUserMenuPage: function () {
                    applicationServiceSet.internalServiceApi.userManagement.GetMenusByOrgUIDNew.send([APPMODEL.Storage.getItem('copPage_token'), $stateParams.orgID, $stateParams.UID,2]).then(function (data) {
                        if (data.Ret == 0) {
                ...
                //     applicationServiceSet.internalServiceApi.userManagement.GetSimpleOrgUsers.send([APPMODEL.Storage.getItem('copPage_token'), $stateParams.orgID]).then(function (data) {
                //         if (data.Ret == 0) {
                ...
                addUserMenu:function (item) {
                    applicationServiceSet.internalServiceApi.userManagement.AddUserMenu.send([$stateParams.UID, $stateParams.orgID, item.MenuID,2]).then(function (data) {
                        if (data.Ret == 0) {
                            toastr.success('添加成功！');
                ...
                deleteUserMenu: function (item) {
                    applicationServiceSet.internalServiceApi.userManagement.DeleteUserMenuNew.send([item.MenuID,$stateParams.orgID,$stateParams.UID,2]).then(function (data) {
                        if (data.Ret == 0) {
                            toastr.success('删除成功');
                            userMenuPermissions.serviceApi.getUserMenuPage();
                        }
                ...
                pageIndex: function () {
                    ...
                    $scope.pageIndex = {
                        ...
                        fliPage: function (page) {
                            applicationServiceSet.internalServiceApi.userManagement.GetUserMenuPage.send([APPMODEL.Storage.getItem('copPage_token'), $stateParams.orgID, $scope.model.name, $scope.model.threeMenuName, $scope.model.pSize, page.pIndex]).then(function (data) {
                                if (data.Ret == 0) {
                        ...
                        nextPage: function (pageNext) {
                            applicationServiceSet.internalServiceApi.userManagement.GetUserMenuPage.send([APPMODEL.Storage.getItem('copPage_token'), $stateParams.orgID, $scope.model.name, $scope.model.threeMenuName, $scope.model.pSize, pageNext]).then(function (data) {
                                if (data.Ret == 0) {
                                    userMenuPermissions.operation.enumOrgTypeChange(data.Data.ViewModelList);//enum org type
                                    $scope.pageIndex.pages = data.Data.Pages;//paging pages
                                    $scope.pageIndex.pageindexList(data.Data);//paging
