 ...
 */
app.controller('listOfCloudRegionAdminList', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', '$modal', 'applicationServiceSet', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, $modal, applicationServiceSet) {
    /**
     * 用户管理
        ...
        serviceApi: (function () {
            return {
                ...
                getSimpleOrgUsers: function () {
                    applicationServiceSet.internalServiceApi.userManagement.GetOrgAdmin.send([APPMODEL.Storage.getItem('copPage_token'), $stateParams.id,2]).then(function (data) {
                        if (data.Ret == 0) {
                ...
                addOrgUser: function (item, func) {
                    applicationServiceSet.internalServiceApi.userManagementInstitution.AddOrgUserNew.send([$stateParams.id, item.UID,2], [APPMODEL.Storage.getItem('copPage_token')]).then(function (data) {
                        if (data.Ret == 0) {
