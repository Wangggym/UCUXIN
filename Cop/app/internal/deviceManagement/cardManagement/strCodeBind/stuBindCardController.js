 ...
 */
app.controller('stuBindCardController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', 'applicationServiceSet', '$modal', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, applicationServiceSet, $modal) {
    var ctr = {
        init: function () {
            this.basic();
            this.getDetail();
        ...
        //获取学生绑卡详情
        getDetail: function () {
            applicationServiceSet.attendanceService.basicDataControlService.GetStuCardByUMID.send([$stateParams.ID]).then(function (data) {
                if (data.Ret == 0) {
        ...
        imgFileUpload: function (file, item) {
            applicationServiceSet.attendanceService.basicDataControlService.ImageRegistrationUpload.fileUpload(file).then(function (data) {
                if (data.Ret == 0) {
        ...
        basic: function () {
            ...
            //返回
            $scope.back = function () {
              $state.go("access.app.internal.cardManagement.strCodeBind");
            };
            //保存学生信息
            $scope.save = function () {
            ...
            $scope.fileChange = function (file, item) {
                if (file) {
                    if(file.size > 1048576){ //大于1M
                        toastr.error("照片应小于1M");
                        return;
                    }
