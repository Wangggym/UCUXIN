 ...
 */
app.controller('HeightReferenceController', ['$scope', '$state', 'EzConfirmConfig', 'EzConfirm', 'toastr', 'toastrConfig','applicationServiceSet',  function ($scope, $state, EzConfirmConfig, EzConfirm, toastr, toastrConfig,applicationServiceSet) {
  'use strict';
  ...
  var ctrl = (function () {
    ...
    var getDictHeightPages = function () {

      var monthAge = String(vm.queryFields.monthAge) || 0,
        sex = vm.queryFields.sex || 0;
      service('getDictHeightPages',[token, vm.pagination.PageIndex, vm.pagination.PageSize, monthAge, sex], function (data) {
        if(data.Ret === 0){
    ...
    return {
      getDictHeightPages: getDictHeightPages,
  ...
  var vm = $scope.vm = {
    ...
    save: function (id) {
      $state.go('access.app.internal.nutritionBasicDictionary.saveHeightDictionary', {id:id});
    },
    del: function (id) {
      EzConfirm.create().then(function() {
        ctrl.delDictHeight(id);
      }, function() {});
    }
  };
  vm.init();
