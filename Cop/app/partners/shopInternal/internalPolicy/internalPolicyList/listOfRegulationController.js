            ...
            return {
                ...
                dataChangeBaseRule: function (data) {
                    for (var i in data ) {
                        ...
                        if(data[i].LimitPoint==0){
                            data[i].LimitPointName='无上限';
                        }
                        else {
                            data[i].LimitPointName=data[i].LimitPoint;
                        }
                    }
...
app.controller('newAddBaseRule', ['$scope', '$modalInstance', 'items', 'service', 'applicationServiceSet', function ($scope, $modalInstance, items, service, applicationServiceSet) {
    ...
    $scope.limitTypeChange=function () {
        ...
        if($scope.newModel.LimitType==0){

            $scope.newModel.LimitPoint=undefined;
        }
    }
    setTimeout(function () {
