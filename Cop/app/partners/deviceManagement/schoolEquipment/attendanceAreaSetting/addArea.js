...
app.controller('addArea', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$location', '$rootScope', '$window', 'toastr', 'applicationServiceSet', '$modal', function ($scope, $http, $state, $stateParams, $timeout, $location, $rootScope, $window, toastr, applicationServiceSet, $modal) {
  var ctrl = {
    ...
    AddOrUpClockArea: function () {
      applicationServiceSet.attendanceService.basicDataControlService.AddOrUpClockArea.send(["",$scope.model.selectedGid,$scope.model.areaName,$scope.model.clockType,$scope.model.attendanceTarget,$scope.selectedClassList]).then(function (data) {
        if (data.Ret == 0) {
          toastr.success("添加成功");
          $state.go("access.app.partner.schoolEquipment.attendanceAreaSetting");
        }
      })
    },
