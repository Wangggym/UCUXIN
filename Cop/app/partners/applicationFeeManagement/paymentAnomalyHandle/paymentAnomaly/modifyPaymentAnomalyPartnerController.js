 ...
 */
app.controller('ModifyPaymentAnomalyPartnerController', ['$scope', '$rootScope', '$modal', '$location', 'toastr', 'applicationServiceSet', function ($scope, $rootScope, $modal, $location, toastr, applicationServiceSet) {
  'use strict';
  // 查询字段
  $scope.queryFields = {
    school: undefined,
  ...
  var modifyPaymentAnomaly = (function () {
    ...
    var getStuList = function (classId, text) {
      getService('getStudentInfoList', [token, $scope.queryFields.school, classId, text], function (data) {
        // 此处
        if (data.Ret === 0) {
    ...
    var upHandleExceptionStu = function (arr) {
      getService('upHandleExceptionStu', arr, function (data) {
        if (data.Ret == 0) {
          ...
          $location.path('/access/app/partner/paymentAnomalyHandle/paymentAnomaly');
        }
      });
    };
  ...
  $scope.query = function () {
      if(!$scope.queryFields.classId || $scope.queryFields.classId == ''){
          toastr.error('请先选择班级！');
          return;
      }
  ...
  applicationServiceSet.chargeServiceApi.chargeService.HandleExceptionStuNew.send([md.ID,$scope.queryFields.school, md.NewUName, md.NewMID, md.NewTel, md.NewClassName, md.NewUMID, md.NewClassID, md.Desc,md.NewChargeID]).then(function (data) {
      if (data.Ret == 0) {
          toastr.success('修改成功！');
          $location.path('access/app/partner/paymentAnomalyHandle/paymentAnomaly');
      }
  });
    //modifyPaymentAnomaly.upHandleExceptionStudent([md.ID, md.NewGid, md.NewUName, md.NewMID, md.NewTel, md.NewClassName, md.NewUMID, md.NewClassID, md.Desc]);
  };
