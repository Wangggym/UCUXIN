...
app.controller('SmsPushController', ['$scope', '$modal', '$location', 'applicationServiceSet', function ($scope, $modal, $location, applicationServiceSet) {
    ...
    var smsPush = (function () {
        ...
        var getPushRecord = function () {
            ...
            getService('getPushRecordPage', [token, $scope.pagination.currentPage, $scope.pagination.itemsPerPage, 2, orgid, schoolId], function (data) {
                if (data.Ret == 0) {
    ...
    $scope.addPush = function () {
        $location.path('/access/app/partner/paymentMessage/addSmsPush');
    };
    /**
     * 导出
     */
    $scope.export = function () {
        window.location.href = urlConfig + 'Charge/v3/Charge/ExportPushRecord' + '?token=' + APPMODEL.Storage.getItem("copPage_token") + '&pushType=2';
    };
