...
class PropertyAssemblage extends Component {
  ...
  getBaseProperties = (property) => {
    ...
    // 查询参数配置
    const options = {Property: property};
    ...
    ServiceAsync('GET', 'Resource/v3/Property/GetBasePropertyRelation', {token, ...options}).then(res => {
      if (res.Ret === 0) {
        if (res.Data) {
          const data = res.Data;
  ...
  getBasePropertyID = () => {
    ...
    ServiceAsync('GET', 'Resource/v3/Property/GetBasePropertyID', {token, ...query}).then(res => {
      if (res.Ret === 0) {
        if (res.Data) {
          this.setState({
  ...
  getAdditional = () => {
    ...
    ServiceAsync('GET', 'Resource/v3/Property/GetAppendPropertyRelation', {token, ...options}).then(res => {
      if (res.Ret === 0) {
        if (res.Data) {
          const data = res.Data;
          Object.keys(data).forEach(key => {
  ...
  render() {
    ...
    return (
      showPropertyAssemblage() ? <div className={`${className ? className + " " : ''}form-search-group`}>
        {
