...
const Footer = ({onClick, onFavorClick, onTranspondClick,signInClick, IsFavor, isTranspond, TotalFee, QtyLimit, QtySignUp, children, IsBuy,TeachWay}) => {
    const buttonStatus = () => {
      ...
      if(IsBuy){
        ...
        return <div className={"goto-study"} onClick={() => onClick(false)}> 已购买</div>;
      }else if(IsBuy&&IsBuy&&TeachWay===3){
        return <div className="sign-in" onClick={() => signInClick()}>一键签到</div>;
      }
        if(QtyLimit && QtyLimit === QtySignUp) return <div className={"grey-background goto-study"} onClick={() => onClick(false)}> 名额已满</div>;
        return <div className={"goto-study"} onClick={() => onClick(true,TotalFee)}> 我要学习</div>
    }
    return (
