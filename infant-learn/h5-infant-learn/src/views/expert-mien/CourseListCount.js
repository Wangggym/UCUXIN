...
import { PullRefreshWrappedComp, NoAccordionCourse, T_Time, NoContent,Course } from '../../components'
...
export default class PlanList extends React.Component {
    ...
    render() {
        ...
        return (
            <div>
                {ViewModelList && ViewModelList.length ? ViewModelList.map((item, i) => <div key={i} className='plan'>
                    <Course {...item} />
                    <div className='border-bottom1PX' />
                </div>) : <NoContent firstLoading={firstLoading} />}
            </div>
        )
    }
}
