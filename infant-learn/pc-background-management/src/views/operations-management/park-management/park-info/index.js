...
export default class ParkInfo extends React.Component {
    ...
    getGardenInfoByID() {
        ...
        api.OperationsManagement.getGardenInfoByID({ gardenID: this.props.match.params.id }).then(res => {
    ...
    getTeachers(gardenID) {
        api.OperationsManagement.getTeachers({ gardenID }).then(res => {
            this.setState({ loading: false })
            if (!res) {
                return message.error('res为空')
            }
    ...
    getLeaderList(data) {
        ...
        console.log(leaderList, TeacherList)
    }
    ...
    render() {
        const { parkInfo: { Name, ImgUrl, History, CreatorName, Theory, Feature, Hardware, Tel, TeaName, AchievementList, Honor },
            loading, ishasInfo, leaderList, TeacherList } = this.state
        const EditButton = <Button onClick={this.handleEditor} type='primary'>编辑</Button>
        return (
            <Spin spinning={loading}>
                {
                    ...
                    </Tabs> : <div>暂无园所信息，<Link to='/editor-parkInfo'>点击+</Link>创建园所信息，展示园所吧</div>
                }
            </Spin>
        )
    }
