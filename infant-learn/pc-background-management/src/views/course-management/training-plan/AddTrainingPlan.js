...
class AddTrainingPlan extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      TrainType: [],//存储培训类型
      TrainObj: [], //存储培训对象
      TrainningObjValue: [],//存储选择培训对象的值
      //plainOptions: [],//存储培训对象文本
      TeachWay: [],//存储授课方式
      TeachWayplainOptions: [],//存储授课方式文本
      IsCharge: false,//是否可收费
      visible: false,//是否显示选择讲师模态框
      ...
      loading: false,
      saveButton:false,//保存按钮状态（限制不能够重复提交）
    }
  }
...
//获取省市县
  ...
  handleSearch = (e) => {
    ...
    e.preventDefault();
    const {TrainningObjValue, HaveSelectTeacher, RangList, TrainObj, ProCityRangList} = this.state;
    //c处理选择的培训对象的值
    let str = 0;
    if (TrainningObjValue) {
      TrainningObjValue.forEach(item => {
        str += +item
      })
    }
    this.props.form.validateFields((err, values) => {
      ...
      SaveData.AttachName = this.state.fileList.length !== 0 ? this.state.fileList[0].name : "";//附件名称
...
//搜索
  ...
  sureOrganization(ProCityRangList) {
    ...
    if (!this.state.limitNum) {
      message.warn("请先填写限制人数");
      this.setState({organizationModal: false});
      return;
    } else {
      if (this.state.limitNum < (schoolNum ? schoolNum : 0) + setNum) {
        message.warn(`总人数(${(schoolNum ? schoolNum : 0) + setNum})不能超过限制人数(${this.state.limitNum})！`)
        return;
      }
    }
  ...
  sureAddShool() {
    ...
    if (!this.state.limitNum) {
      ...
      if (this.state.limitNum < ((setNum ? setNum : 0) + schoolNum)) {
        message.warn(`总人数(${(setNum ? setNum : 0) + schoolNum})不能超过限制人数(${this.state.limitNum})！`);
        return;
      }
    }
...
//过滤调人数为空的学校
  ...
  handleUploadHeadPic = (file, token) => {
    ...
    Api.Base.upload({
      ...
      this.setState({uploadIDPic: false}, () => message.success("上传成功"));
    })
  };
  ...
  limitTime = (selectDate) => {
    if (selectDate) this.setState({trainStartDate: selectDate.valueOf()});
  };
  //打开日期选择面板
  openStartEnd = () => {
  ...
  render() {
    const {TrainType, uploadIDPic, TrainObj, TeachWay, ManageLevel, addShool, ProCityRangList, ProvinceData, visible, TeacherInfo, HaveSelectTeacher, areas, headPic, organizationModal, organization, RangList} = this.state;
        ...
        <Form className="form-search-group" onSubmit={this.handleSearch}>
          ...
          <FormItem {...formItemLayout} label={'限制人数'}>
            {getFieldDecorator(`QtyLimit`, {rules: [{required: true, message: '请填写本计划限制人数'}]})(
              <InputNumber onChange={(value) => this.setState({limitNum: value})} placeholder="限制人数" min={0}
                           precision={0}/>
            )}
          </FormItem>
          ...
          <FormItem {...formItemLayout} label="可见范围">
            {getFieldDecorator(`SchoolScopeName`)(
              <Input placeholder="请选择可见范围 " disabled className="scope-name"
                      ...
                      return (
                        <Row key={item.ID} style={{marginBottom: "0.5rem"}}>
                          ...
                          <Col span={10} style={{display: "flex", alignItems: "center", height: "2rem"}}>
                            限定名额&nbsp;&nbsp;
                            <InputNumber placeholder="人数" min={0} precision={0}
          ...
          <Spin spinning={this.state.loading} className="search-loading">
            ...
            >
              ...
              <div className="content">
                {
                  organization.length ? organization.map((item, index) => {
                    return (
                      <Row key={item.ID} style={{marginBottom: "0.5rem"}}>
                        <Col span={width}/>
                        <Col span={8} style={{display: "flex", alignItems: "center", height: "2rem"}}>
                          {item.FName}:{item.Count}
                        </Col>
                        <Col span={10} style={{display: "flex", alignItems: "center", height: "2rem"}}>
                          限定名额&nbsp;&nbsp;
          ...
          <FormItem {...formItemLayout} label={'培训总学分'}>
            {getFieldDecorator(`TrainingTotalScore`)(
              <InputNumber placeholder="培训总学分" min={0}/>
            )}
          </FormItem>
          ...
          <FormItem {...formItemLayout} label={'授课讲师'}>
            {getFieldDecorator(`Teacher`, {rules: [{required: true, message: '请选择授课讲师'}]})(
              <Input disabled placeholder="请选择授课讲师 "
                     addonAfter={<Icon onClick={() => this.selectTeacher()} type="user-add"/>}/>
            )}
            <Modal
              width="80%"
              visible={visible}
              ...
              <Row>
                <Col span={14}>
                  <FormItem {...formItemLayout} label={'讲师名称'}>
                    <Search onSearch={value => this.searchTeacher(value)} onKeyPress={(e) => this.onKeyPress(e)}
                  ...
                  <Card style={{width: "100%"}} title="所有讲师">
                    {
                      TeacherInfo.length ? TeacherInfo.map((item, key) => {
                        return (
                          <div className="teacher-info" key={item.ID} onClick={() => this.addTeacher(item)}>
                            <p>
                  ...
                  <Card style={{width: "100%"}} title="已选讲师">
                    {
                      HaveSelectTeacher.length ? HaveSelectTeacher.map((item, key) => {
                        return (
                          <div className="teacher-info" key={item.ID} onClick={() => this.delHaveSelect(item)}>
                            <p>
                              <img src={item.HeadPic} alt={item.Name}/>
                            </p>
                            <span>{item.Name}</span>
          ...
          <div className={this.state.IsCharge ? "current-block" : "current-none"}>
            <FormItem {...formItemLayout} label={'按学分收费'}>
              {getFieldDecorator(`AccordingScore `)(
                <InputNumber placeholder="按学分收费 " min={0}/>
              )}
            </FormItem>
            <FormItem {...formItemLayout} label={'按培训项目收费'}>
              {getFieldDecorator(`AccordingTrain`)(
                <InputNumber placeholder="总收费 " min={0}/>
              )}
            </FormItem>
          </div>

          <FormItem {...formItemLayout} label="课程提交截至时间">
            {getFieldDecorator(`CourseEDate`)(<DatePicker
              disabledDate={this.disabledEndDate}
              allowClear={false}
              onChange={this.limitTime}
              format={dateFormat} showTime={{format: 'HH:mm:ss'}}/>)}
          </FormItem>

          <FormItem {...formItemLayout} label="培训起止时间">
            {getFieldDecorator(`TrainSEDate`)(<RangePicker
              onOpenChange={this.openStartEnd}
              open={this.state.isOpen}
              format={dateFormat} disabledDate={this.disabledRangeDate} showTime={{format: 'HH:mm:ss'}}/>)}
          </FormItem>
          ...
          <Row>
            ...
            <Col span={2}>
              <Button type="primary" style={{marginLeft: 8}} htmlType="submit" disabled={this.state.saveButton}>保存</Button>
            </Col>
          </Row>
        </Form>
      </div>
