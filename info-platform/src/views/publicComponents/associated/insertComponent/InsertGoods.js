...
class InsertGoods extends Component{
  ...
  constructor(props){
    ...
    this.state = {
      ...
      Keyword:'',
      data:{ContentType:25,ContentID:1,Title:null,Thumb:null,Img:null,Url:null},
    };
    this.colums = [
      {
        title: '商品名称',
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.dataClean && nextProps.dataClean !== this.props.dataClean){
      ...
      this.setState({data:{ContentType:25,ContentID:1,Title:null,Thumb:null,Img:null,Url:null}})
    }
  }
        ...
        <Form>
          <FormItem  {...formItemLayout}>
            {getFieldDecorator('search', {
              rules: [{ required: false, message: '请搜索商品名称' }],
            })(
              <Search
                placeholder="请搜索商品名称"
                onSearch={value => {
                  this.setState({Keyword: value}, () => {this.getGoodsList(1)});
                }}
              />
            )}
          </FormItem>
          ...
          <FormItem label="上传图片" {...formItemInput} extra="该图片会出现在图集的最后,点击会打开关联的内容">
          ...
          <FormItem  {...tailFormItemLayout}>
            <Button style={{float:'right'}} type="primary" onClick={(e)=>this.sureGoods(e)}>确定</Button>
...
export default Form.create()(InsertGoods);